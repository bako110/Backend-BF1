...
class SeriesService:
    ...
    async def get_all_series(self, page: int = 1, limit: int = 20, status: str = None, is_premium: bool = None) -> dict:
        ...
        if is_premium is not None:
            filters.append(Series.is_premium == is_premium)
        
        if filters:
            query = Series.find(*filters)
        else:
