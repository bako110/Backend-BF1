...
async def get_all_tickets_admin(
    status_filter: Optional[str] = Query(None, description="Filtrer par statut"),
    category_filter: Optional[str] = Query(None, description="Filtrer par catégorie"),
    limit: int = Query(100, ge=1, le=500),
    skip: int = Query(0, ge=0),
    current_user: User = Depends(get_admin_user)
):
    """Récupérer tous les tickets de support (Admin uniquement)"""
    ...
    if category_filter:
        query = query.find(SupportTicket.category == category_filter)
    
    tickets = await query.sort(-SupportTicket.created_at).skip(skip).limit(limit).to_list()
    return tickets
