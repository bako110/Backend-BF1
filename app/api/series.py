...
async def get_all_series(
    ...
    """Récupérer toutes les séries avec pagination (accès public)"""
    try:
        result = await series_service.get_all_series(page, limit, status, is_premium)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/series/{series_id}", response_model=SeriesOut)
async def get_series(series_id: str, current_user=Depends(get_optional_user)):
    """Récupérer une série par son ID (accès public)"""
...
async def get_seasons_by_series(
    ...
    """Récupérer toutes les saisons d'une série (accès public)"""
    try:
...
async def get_episodes_by_series(
    ...
    """Récupérer tous les épisodes d'une série avec pagination (accès public)"""
    try:
...
async def get_episodes_by_season(
    ...
    """Récupérer tous les épisodes d'une saison (accès public)"""
    try:
